...
That inbox lives under the remote home's private parent-route state directory, whose shape [`bin/fm-parent-route-lib.sh`](../bin/fm-parent-route-lib.sh) owns.
The charter published to that host names the inbox there, so a mate checking its own inbox at a checkpoint or at startup reads a path that exists on its own machine rather than one belonging to the supervising home.
